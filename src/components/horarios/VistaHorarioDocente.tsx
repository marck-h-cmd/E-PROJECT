...
export default function VistaHorarioDocente({ docenteId, periodoId }: Props) {
  const [matriz, setMatriz] = useState<Record<string, Record<string, Sesion>>>({});
  const [loading, setLoading] = useState(true);
  const [docente, setDocente] = useState<{ nombres: string; apellidos: string } | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/docentes/${docenteId}`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
    ])
      .then(([horarios, doc]) => {
        setDocente(doc.data ?? null);
    ...
    <div className="card overflow-hidden">
      <div className="card-header">
        <h3 className="text-sm font-semibold text-gray-900">
          Horario: {docente ? `${docente.apellidos}, ${docente.nombres}` : 'Docente'}
        </h3>
      </div>
              ...
              : HORAS.map(hora => (
                  <tr key={hora}>
                    ...
                    {DIAS.map(dia => {
                      ...
                      return (
                        <td key={dia}
                          className={`border p-1.5 ${s ? (s.esPractica ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200') : 'bg-white border-gray-200'}`}
                          style={{ height: 52 }}
                        >
                          {s && (
                            <div className="leading-tight overflow-hidden">
                              <div className="font-semibold text-gray-800 truncate">{s.cursoNombre}</div>
                              <div className="text-gray-500">{s.ambienteCodigo}</div>
