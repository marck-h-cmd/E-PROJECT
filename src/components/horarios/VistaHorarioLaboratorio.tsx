...
interface Sesion { cursoNombre: string; docenteApellidos: string; estado: string; esPractica?: boolean; }

interface Props { laboratorioId: string; periodoId?: string; }

export default function VistaHorarioLaboratorio({ laboratorioId, periodoId }: Props) {
  const [matriz, setMatriz] = useState<Record<string, Record<string, Sesion>>>({});
  const [loading, setLoading] = useState(true);
  const [lab, setLab] = useState<{ codigo: string; nombre: string } | null>(null);

  useEffect(() => {
    const token = localStorage.getItem('token');
    const params = new URLSearchParams({ ambienteId: laboratorioId });
    if (periodoId) params.append('periodoId', periodoId);
    Promise.all([
      fetch(`/api/horarios?${params}`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
      fetch(`/api/ambientes/${laboratorioId}`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
    ])
      .then(([horarios, amb]) => {
        setLab(amb.data ?? null);
  ...
  }, [laboratorioId, periodoId]);

  return (
    <div className="card overflow-hidden">
      <div className="card-header">
        <h3 className="text-sm font-semibold text-gray-900">
          Laboratorio: {lab ? `${lab.codigo} — ${lab.nombre}` : '...'}
        </h3>
      </div>
                      ...
                      return (
                        <td key={dia}
                          className={`border p-1.5 ${
                            s ? (s.esPractica ? 'bg-green-50 border-green-200' : 'bg-teal-50 border-teal-200')
                        ...
                        >
                          {s && (
                            <div className="leading-tight overflow-hidden">
                              ...
                              {s.esPractica && <div className="text-[9px] text-green-600">PRÁCTICA</div>}
                            </div>
                          )}
                        </td>
                      );
